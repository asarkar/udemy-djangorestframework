...
class TokenAuthTests(APITestCase):
    """Tests for token authentication and automatic token creation."""
    ...
    def setUp(self) -> None:
        ...
        self.passenger_list_url = reverse("passenger-list")

    def test_token_created_on_user_creation(self) -> None:
        """Test that a token is automatically created when a user is created."""
        self.assertTrue(Token.objects.filter(user=self.user).exists())

    def test_unauthenticated_flight_list_denied(self) -> None:
        """Test that unauthenticated users cannot access flights."""
        response = self.client.get(self.flight_list_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_unauthenticated_reservation_list_denied(self) -> None:
        """Test that unauthenticated users cannot access reservations."""
        response = self.client.get(self.reservation_list_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_authenticated_with_token_can_access_flights(self) -> None:
        """Test that users with valid token can access flights."""
        self.client.credentials(HTTP_AUTHORIZATION=f"Token {self.token.key}")
        response = self.client.get(self.flight_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_authenticated_with_token_can_access_reservations(self) -> None:
        """Test that users with valid token can access reservations."""
        self.client.credentials(HTTP_AUTHORIZATION=f"Token {self.token.key}")
        response = self.client.get(self.reservation_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_passenger_endpoint_no_auth_required(self) -> None:
        """Test that passenger endpoint doesn't require authentication."""
        response = self.client.get(self.passenger_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)


class FlightValidationTests(APITestCase):
    """Tests for flight field validations."""

    def setUp(self) -> None:
        self.url = reverse("flight-list")
        # Authenticate for protected endpoint
    ...
    def test_create_flight_valid_flight_number(self) -> None:
        """Test creating a flight with valid alphanumeric flight number."""
        response = self.client.post(self.url, self.flight_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["flight_number"], "AA100")

    def test_create_flight_flight_number_with_special_chars(self) -> None:
        """Test that flight number with special characters is rejected."""
        data = {**self.flight_data, "flight_number": "AA-100"}
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("flight_number", response.data)

    def test_create_flight_flight_number_with_spaces(self) -> None:
        """Test that flight number with spaces is rejected."""
        data = {**self.flight_data, "flight_number": "AA 100"}
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("flight_number", response.data)

    def test_create_flight_flight_number_with_unicode(self) -> None:
        """Test that flight number with non-ASCII characters is rejected."""
        data = {**self.flight_data, "flight_number": "AA١٠٠"}  # Arabic numerals
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("flight_number", response.data)

    def test_create_flight_flight_number_only_letters(self) -> None:
        """Test that flight number with only letters is valid."""
        data = {**self.flight_data, "flight_number": "AABC"}
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_flight_flight_number_only_digits(self) -> None:
        """Test that flight number with only digits is valid."""
        data = {**self.flight_data, "flight_number": "1234"}
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_flight_flight_number_with_underscore(self) -> None:
        """Test that flight number with underscore is rejected."""
        data = {**self.flight_data, "flight_number": "AA_100"}
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("flight_number", response.data)
